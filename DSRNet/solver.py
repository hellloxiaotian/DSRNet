...
class Solver():
    def __init__(self, model, cfg):
        if cfg.scale > 0:
            self.refiner = model(args=cfg)
        else:  # there is mutile scales
        ...
        # self.writer = print(os.path.join("runs:", cfg.ckpt_name))
        if cfg.verbose:
            num_params = 0
        ...
        while True:
            for inputs in self.train_loader:
                ...
                if cfg.verbose and self.step % cfg.print_interval == 0:
                    if cfg.scale > 0:
                        psnr = self.evaluate("datasets/DIV2K", scale=cfg.scale, num_step=self.step)
    ...
    def evaluate(self, test_data_dir, scale=4, num_step=0):
        ...
        for step, inputs in enumerate(test_loader):  # step is the number of test images
            ...
            sr = result
            hr = hr.cpu().mul(255).clamp(0, 255).byte().permute(1, 2,
                                                                0).numpy()  # (644.1024,3) is the same dimensional with the size of input test image in dataset.py.
